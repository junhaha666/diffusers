...
def get_cdna_autotune_fwd_configs():
    return [
        ...
        'ACTUAL_BLOCK_DMODEL', 'VARLEN', 'HQ', 'HK', "USE_FP8", "BLOCK_M"
    ]
...
def attn_fwd(Q, K, V, bias, p_scale: tl.constexpr, q_descale_ptr,
             k_descale_ptr, v_scale_ptr, USE_FP8: tl.constexpr,
    ...
    num_block_n: tl.constexpr = (MAX_SEQLENS_K + BLOCK_N - 1) // BLOCK_N
...
def _bwd_preprocess_use_o(
    ...
    if USE_FP8:
        do_scale = 57344. / (tl.max(tl.abs(do)) + 1e-7)
        do_fp8 = tl.clamp(do * do_scale, -57344, 57344).to(FP8_E5_DTYPE)
...
def _bwd_kernel_one_col_block(
    ...
    mask_d = offs_d < (ACTUAL_BLOCK_DMODEL - OFFSET_DMODEL)
    kv_mask_0 = mask_n[None, :]
    kv_mask_1 = mask_n[None, :] & mask_d[:, None]
    
    k_ptrs = k_offset + offs_n[None, :] * stride_kn + offs_d[:,
                                                             None] * stride_kk
    k0 = tl.load(k_ptrs, mask=kv_mask_0, other=0.0)

    v_ptrs = v_offset + offs_n[None, :] * stride_vn + offs_d[:,
                                                             None] * stride_vk
    v0 = tl.load(v_ptrs, mask=kv_mask_0, other=0.0)

    k_ptrs = k_ptrs + OFFSET_DMODEL * stride_kk
    k1 = tl.load(k_ptrs, mask=kv_mask_1, other=0.0)

    v_ptrs = v_ptrs + OFFSET_DMODEL * stride_vk
    v1 = tl.load(v_ptrs, mask=kv_mask_1, other=0.0)
    ...
    for start_m in range(lo, num_block_m * BLOCK_M, BLOCK_M):
        ...
        q_mask_0 = mask_m[:, None]
        q_mask_1 = mask_m[:, None] & mask_d[None, :]
        
        q0 = tl.load(q_ptrs, mask=q_mask_0, other=0.0)
        qk = tl.dot(q0, k0, acc=qk, out_dtype=tl.float32)

        q_ptrs = q_ptrs + OFFSET_DMODEL * stride_qk
        q1 = tl.load(q_ptrs, mask=q_mask_1, other=0.0)
        ...
        do_ptrs = do_offset + offs_m[:, None] * stride_qm + offs_d[
            None, :] * stride_qk
        do0 = tl.load(do_ptrs, mask=q_mask_0)

        do_ptrs = do_ptrs + OFFSET_DMODEL * stride_qk
        do1 = tl.load(do_ptrs, mask=q_mask_1)
        ...
        Di = tl.load(d_ptrs, mask=mask_m)  # D stored in fp32
        ...
        if SEQUENCE_PARALLEL:
            dq_mask_0 = mask_m[None, :]
            dq_mask_1 = mask_m[None, :] & mask_d[:, None]
            ...
            dq_ptrs = dq_offset + offs_m[
                None, :] * stride_qm + offs_d[:, None] * stride_qk
            tl.atomic_add(dq_ptrs, dq, mask=dq_mask_0, sem='relaxed')
            ...
            tl.atomic_add(dq_ptrs, dq, mask=dq_mask_1, sem='relaxed')

        else:
            dq_ptrs = dq_offset + offs_m[:, None] * stride_qm + offs_d[
                None, :] * stride_qk
            dq = tl.load(dq_ptrs, mask=q_mask_0, other=0.0)
            ...
            dq += _dq
            tl.store(dq_ptrs, dq, mask=q_mask_0)

            dq_ptrs = dq_ptrs + OFFSET_DMODEL * stride_qk
            dq = tl.load(dq_ptrs, mask=q_mask_1, other=0.0)
            ...
            dq += _dq
            tl.store(dq_ptrs, dq, mask=q_mask_1)
    ...
    dv_mask_0 = mask_n[:, None]
    dv_mask_1 = mask_n[:, None] & mask_d[None, :]
    # write-back
    if GROUP_SIZE != 1:
        dk_ptrs = dk_offset + offs_n[
            None, :] * stride_kn + offs_d[:, None] * stride_kk
        tl.atomic_add(dk_ptrs, dk0, mask=kv_mask_0, sem='relaxed')
        dk_ptrs = dk_ptrs + OFFSET_DMODEL * stride_kk
        tl.atomic_add(dk_ptrs, dk1, mask=kv_mask_1, sem='relaxed')
        dv_ptrs = dv_offset + offs_n[:, None] * stride_vn + offs_d[
            None, :] * stride_vk
        tl.atomic_add(dv_ptrs, dv0, mask=dv_mask_0, sem='relaxed')
        dv_ptrs = dv_ptrs + OFFSET_DMODEL * stride_vk
        tl.atomic_add(dv_ptrs, dv1, mask=dv_mask_1, sem='relaxed')
    else:
        dk_ptrs = dk_offset + offs_n[
            None, :] * stride_kn + offs_d[:, None] * stride_kk
        tl.store(dk_ptrs, dk0, mask=kv_mask_0)
        dk_ptrs = dk_ptrs + OFFSET_DMODEL * stride_kk
        tl.store(dk_ptrs, dk1, mask=kv_mask_1)
        dv_ptrs = dv_offset + offs_n[:, None] * stride_vn + offs_d[
            None, :] * stride_vk
        tl.store(dv_ptrs, dv0, mask=dv_mask_0)
        dv_ptrs = dv_ptrs + OFFSET_DMODEL * stride_vk
        tl.store(dv_ptrs, dv1, mask=dv_mask_1)
...
def get_cdna_autotune_bwd_configs():
    return [
        ...
        "BLOCK_DMODEL", "ACTUAL_BLOCK_DMODEL", "GROUP_SIZE", "SEQUENCE_PARALLEL", "CAUSAL",
        "USE_FP8", "BLOCK_M"
    ]
...
def _bwd_kernel(
    ...
    if SEQUENCE_PARALLEL:
        ...
        num_block_n: tl.constexpr = (N_CTX_K + BLOCK_N - 1) // BLOCK_N
...
def attention_block_backward_triton_impl(
    ...
    if use_fp8:
        do_fp8 = torch.empty(do.shape, dtype=get_e5m2_dtype(), device=q.device)
    ...
    _bwd_preprocess_use_o[grid_prebwd](
        ...
        HQ=nheads_q,
        IS_VARLEN=is_varlen,
        FP8_E5_DTYPE=tl.float8e5b16 if is_hip() else tl.float8e5)
...
def block_scaling_node(tensor, BLOCK_M=FIXED_BLOCK_M, float8_dtype=get_e4m3_dtype()):
